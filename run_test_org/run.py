...
def main():
    ...
    # load the input data
    print('\nLoading Dataset...')
    traffic = pd.read_csv(os.path.join(data_dir, 'train.csv')).reset_index()# 1. reset_index*4
    traffic = traffic.rename(columns={'index': 'traffic_index'})
    print('a ', traffic['datetime'].head(3))
    search_spec = pd.read_csv(os.path.join(data_dir, 'search_data.csv')).reset_index()
    search_spec = search_spec.rename(columns={'index': 'search_spec_index'})
    search_unspec = pd.read_csv(os.path.join(data_dir, 'search_unspec_data.csv')).reset_index()
    search_unspec = search_unspec.rename(columns={'index': 'search_unspec_index'})
    ic_master = pd.read_csv(os.path.join(data_dir, 'road.csv')).reset_index()
    ic_master = ic_master.rename(columns={'index': 'road_index'})
    ...
    search_spec['datetime'] -= pd.to_timedelta(1, 'd')# 当日へ
    search_unspec['date'] -= pd.to_timedelta(1, 'd')# 当日へ
    search_spec['datetime'] = search_spec['datetime'].astype('str')
    search_unspec['date'] = search_unspec['date'].astype('str')

    df = make_dataset(traffic, ic_master, search_spec, search_unspec)# ここ注目
    ...
    for d in tqdm(valid['datetime'].dt.date.unique()):# shiftしながら予測していく.
